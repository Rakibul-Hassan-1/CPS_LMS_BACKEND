...
module.exports = {
  ...
  async listRoles(ctx) {
    const roles = await strapi
      ...
      .findMany({
        where: {
          type: { $in: ["admin", "content_manager", "instructor", "student"] },
        },
      });
    return { data: roles };
  },
  ...
  async updateUserRole(ctx) {
    ...
    if (!allowedTypes.includes(roleType)) {
      return ctx.badRequest(
        `roleType must be one of: ${allowedTypes.join(", ")}`,
      );
    }
    ...
    if (!role)
      return ctx.notFound("Role not found. Did the server bootstrap run?");
  ...
  async deleteUser(ctx) {
    ...
    if (String(ctx.state.user.id) === String(id)) {
      return ctx.badRequest("You can't delete your own account.");
    }
  ...
  async stats(ctx) {
    ...
    const totalEnrollments = await strapi.entityService.count(
      "api::enrollment.enrollment",
    );
    const totalQuizzes = await strapi.entityService.count("api::quiz.quiz");
    const totalBlogPosts = await strapi.entityService.count(
      "api::blog-post.blog-post",
    );
    const publishedBlogPosts = await strapi.entityService.count(
      "api::blog-post.blog-post",
